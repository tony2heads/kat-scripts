...
if __name__ == "__main__":
    ...
    k = Sun()
#
#    lat=52.9147                    #Westerbork latitude
#    long=6.6034                    #   and longitude
    lat= -30.71317                  # MeerKAT latitude
    long= 21.44389                  # and long
    year=gmtime()[0]               #get current year
    yyyy=str(year)+" "
#   month=gmtime()[1]
#   day=gmtime()[2]

    for doy in range(1,365):            #loop over days of the relevant year
         date=strptime(yyyy+str(doy),"%Y %j")
         month=date[1]
         day=date[2]
#        print k.sunRiseSet(2002, 3, 22, 25.42, 62.15)
         rise=k.sunRiseSet(year, month, day, long, lat)[0]
         set=k.sunRiseSet(year, month, day, long, lat)[1]
         daylight=set-rise
         print strftime("Week%W Day%j %a %d %b", date),
         print "%4d/%02d/%02d: Sunrise %5.2f Set %5.2f UT"%(year,month,day,rise,set),
